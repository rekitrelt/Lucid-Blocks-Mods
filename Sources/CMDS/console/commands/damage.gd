...
func _execute(args: Array[String]) -> void:
	if args.size() != 1:
		console.print_line("[color=red]Usage: damage <hp>[/color]")
		return
	...
	var damage: float = args[0].to_float()

	console.print_line("[color=green]Damaging player (%s)[/color]" % [damage])

	Ref.player.take_damage(damage, 0)
