...
func _execute(args: Array[String]) -> void:
	if args.size() != 1:
		console.print_line("[color=red]Usage: maxhp <amount>[/color]")
		return
	...
	var hp: float = args[0].to_float()

	console.print_line("[color=green]Setting player MaxHealth to (%s)[/color]" % [hp])

	Ref.player.max_health = hp
